...
namespace DataAccess.Repository
{
    public class Repositories<TEntity> : IRepositories<TEntity> where TEntity : class, IEntity
    {
        ...
        {
            await Task.Run(() => context.Remove(AsyncGetSingle(where).Result));
        }
        ...
        public async Task<IList<TEntity>> AsyncGetAll(Expression<Func<TEntity, bool>> where = null, params Expression<Func<TEntity, object>>[] include)
        {
            ...
            if (where != null)
            {
                query = context.Set<TEntity>().Where(where); // Hangi NESNE gönderildiyse O NESNE'nin veritabanındaki Dataları aktaracak.
            }
            else
            {
            ...
            if (include.Any()) //ilişkili tablo istenmiş mi bakılıyor.
            {
        ...
        public async Task<TEntity> AsyncGetSingle(Expression<Func<TEntity, bool>> where, params Expression<Func<TEntity, object>>[] include)
        {
